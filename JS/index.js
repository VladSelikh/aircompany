...
const { EXPEREMENTAL_TYPE } = require("./models/ExperimentalType");
const { CLASSIFICATION_LEVEL } = require("./models/ClassificationLevel");
const { getDisplayView } = require("./plugin/DisplayView");
...
let planes = [
  ...
  new ExperimentalPlane({ model: "Bell X-14", maxSpeed: 277, maxFlightDistance: 482, maxLoadCapacity: 500, experimentalType: EXPEREMENTAL_TYPE.HIGH_ALTITUDE, classificationLevel: CLASSIFICATION_LEVEL.SECRET}),
  new ExperimentalPlane({ model: "Ryan X-13 Vertijet", maxSpeed: 560, maxFlightDistance: 307, maxLoadCapacity: 500, experimentalType: EXPEREMENTAL_TYPE.VTOL, classificationLevel: CLASSIFICATION_LEVEL.TOP_SECRET}),
];
...
(function run() {
  ...
  console.log(
    `Military airport sorted by max distance: ${getDisplayView(
      militaryAirport.sortByMaxDistance()
    )}`
  );
  console.log(
    `Passenger airport sorted by max speed: ${getDisplayView(
      passengerAirport.sortByMaxSpeed()
    )}`
  );
  console.log(
    `Plane with max passenger capacity: ${getDisplayView(
