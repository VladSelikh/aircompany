...
const { getJsonView } = require('./helpers/getJsonView');
const { MILITARY_TYPE } = require('./constants/militaryType');
const { EXPERIMENTAL_TYPES } = require('./constants/experimentalTypes');
const { CLASSIFICATION_LEVEL } = require('./constants/classificationLevel');

(function reportAirportCapacity() {
    let planes = [
        ...
        new ExperimentalPlane({ model: "Ryan X-13 Vertijet", maxSpeed: 560, maxFlightDistance: 307, maxLoadCapacity: 500, type: EXPERIMENTAL_TYPES.VERTICAL_TAKE_OFF, classificationLevel: CLASSIFICATION_LEVEL.TOP_SECRET })
    ];
    ...
    console.log(`Military airport sorted by max distance: ${getJsonView(militaryAirport.getSortedPlanesByMaxDistance())}`);
    console.log(`Passenger airport sorted by max speed: ${getJsonView(passengerAirport.getSortedPlanesByMaxSpeed())}`);
    console.log(`Plane with max passenger capacity: ${getJsonView(passengerAirport.getPassengerPlaneWithMaxPassengersCapacity())}`);
